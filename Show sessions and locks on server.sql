...
SELECT
	...
	(CASE WHEN xs.[status] NOT IN ('sleeping', 'dormant') THEN (CASE WHEN cmd.[encrypted]=1
		...
		ELSE COALESCE(
			SUBSTRING(cmd.[text], xr.statement_start_offset/2+1, NULLIF(xr.statement_end_offset, -1)/2-xr.statement_start_offset/2),
			cmd.[text],
...
SELECT
	...
	(CASE xr.transaction_isolation_level
		...
		ELSE ISNULL(CAST(xr.transaction_isolation_level AS nvarchar(10)), '')
	...
	ISNULL(CAST(dbo.fn_friendly_age(0, DATEADD(ms, (
		...
		WHERE wt.session_id=wt.blocking_session_id AND
			...
			wt.wait_type='CXPACKET'), 0)) AS nvarchar(20)), 'None') AS [Avg cxPkt wait]
...
AS (
	SELECT spid, tran_id, lock_ord, mode, [count], [database], [object_id], [object_name], index_id, index_type, index_name, index_filter,
		...
		CAST((CASE WHEN mode LIKE '%table' THEN '' ELSE LTRIM(ISNULL(dbo.fn_format_number(NULLIF([count], 1), 0)+' ', '')) END)+mode AS nvarchar(max))
	FROM @locks
	WHERE lock_ord=1
	UNION ALL
	SELECT rcte.spid, rcte.tran_id, lck.lock_ord, lck.mode, lck.[count], rcte.[database], rcte.[object_id], rcte.[object_name], rcte.index_id, rcte.index_type, rcte.index_name, rcte.index_filter,
		...
		CAST(rcte.lock_descr+', '+(CASE WHEN lck.mode LIKE '%table' THEN '' ELSE LTRIM(ISNULL(dbo.fn_format_number(NULLIF(lck.[count], 1), 0)+' ', '')) END)+lck.mode AS nvarchar(max))
	FROM @locks AS lck
	INNER JOIN rcte ON
...
SELECT spid AS SPID, tran_id AS [Transaction], lock_descr AS [Lock types],
	...
	ISNULL(STR(NULLIF(partition_number, 0), 4, 0)+'/'+CAST(NULLIF(partition_count, 1) AS nvarchar(10)), '') AS [Part'n],
